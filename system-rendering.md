...
本技能`system-job.example.json`是合成文案示例，必须替换为本项目真实提取内容。顶层：`canvas:[宽,高]`、整数`fps`、默认`style`、`assets`数组。每项：
...
```text
python system-render_overlay.py project-job.json --out renders-v001 --validate-only
python system-render_overlay.py project-job.json --out renders-v001 --ffmpeg /path/to/ffmpeg --font /path/to/cjk-font.ttc --webm
```
